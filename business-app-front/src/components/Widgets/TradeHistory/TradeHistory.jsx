  ...
  return (
      <Paper elevation={3} style={{borderRadius : '15px'}}>
        ...
        <Box sx={{ p: 2 }}>
          <Table size="small" sx={{
            '& .MuiTableCell-root': {
              padding: '8px 16px', // Унифицированный padding для ячеек
              fontSize: '0.875rem' // Единый размер шрифта
            },
            '& .MuiTableBody-root': {
              '& .MuiTableRow-root': {
                '&:not(:last-child)': {
                  borderBottom: '1px solid rgba(224, 224, 224, 0.5)' // Легкая граница между строками
                },
                '&:hover': {
                  backgroundColor: 'action.hover' // Эффект при наведении
                }
              }
            }
          }}>
            <TableHead>
              <TableRow sx={{
                '& .MuiTableCell-head': {
                  fontWeight: 600, // Жирный шрифт для заголовков
                  color: 'text.primary'
                }
              }}>
            ...
            <TableBody>
              {data.map((item) => (
                  <TableRow key={item.id} sx={{
                    '& .MuiTableCell-root': {
                      py: 1 // Вертикальный padding для ячеек тела таблицы
                    }
                  }}>
